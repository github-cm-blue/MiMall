...
import storeage from '../../storage/index'
export default {
  name: "login",
  data() {
    return {
    ...
    login() {
      ...
      this.request({
       ...
       let access_token= this.$cookie.get("access_token");
        //将access_token存入storeage中
        storeage.setItem('access_token',access_token);
        // this.$store.dispatch('saveNickName',response.user.nickName)
        this.saveNickName(response.user.nickName);
